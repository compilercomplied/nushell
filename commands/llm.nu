...
export def q [prompt: string, --backend: string@backends = "gemini"] {
    if $backend == "gemini" {
        call_gemini $prompt
...
def call_gemini [prompt: string] {
    ...
    let headers = {"Content-Type": "application/json", "X-goog-api-key": $env.GEMINI_API_KEY}
...
def call_claude [prompt: string] {
    ...
    let headers = {"x-api-key": $env.ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01", "content-type": "application/json"}
    let response = http post $url ($body | to json) --headers $headers
    | get content.0.text
    render $response
}
