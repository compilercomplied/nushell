...
def "resolve acr subscription" [] {
    ...
    let found_in_current = (
        do -i { az acr show --name $env.acr_name --subscription $current_subscription --query id --output tsv }
    )
...
export def "acr repos" [
    ...
    let repos = (
        az acr repository list --name $env.acr_name --subscription $subscription --output json
        | from json
    )
    if $owned {
